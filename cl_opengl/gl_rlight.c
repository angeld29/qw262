...
float bubblecolor[NUM_DLIGHTTYPES][4] = {
	...
	{ 0.05, 0.45, 0.05, 0.7 },		// green
	{ 0.5, 0.5, 0.5, 0.7},			// white
	{ 0.2, 0.1, 0.05, 0.7 },	// muzzleflash
	{ 0.2, 0.1, 0.05, 0.7 },	// explosion
	{ 0.2, 0.1, 0.05, 0.7},		// rocket (no light bubble)
...
void R_RenderDlight (dlight_t *light)
{
	...
	if (light->type == lt_rocket_trail_1 || light->type == lt_rocket_trail_2) {
		((flare *)light)->color[3] *= 0.5;
		glColor4fv (((flare *)light)->color);
	}
	else
...
// calculate mid point
	...
	if ((back < 0) == side) {
//		return RecursiveLightPoint (node->children[front < 0], start, end);
...
// check for impact on this node
	...
	for (i=0 ; i<node->numsurfaces ; i++, surf++) {
		...
		r = 0;
		if (lightmap) {
			int line3, r00 = 0, r01 = 0,r10 = 0,r11 = 0, dsfrac = ds & 15, dtfrac = dt & 15;

			ds >>= 4;
			dt >>= 4;
			line3 = ((surf->extents[0]>>4)+1);
			lightmap += dt * ((surf->extents[0]>>4)+1) + ds;
			for (maps = 0 ; maps < MAXLIGHTMAPS && surf->styles[maps] != 255 ;	maps++)	{
				scale = d_lightstylevalue[surf->styles[maps]]* 1.0 / 256.0;
				r00 += lightmap[0] * scale;
				r01 += lightmap[1] * scale;
				r10 += lightmap[line3] * scale;
				r11 += lightmap[line3 + 1] * scale;
				lightmap += ((surf->extents[0] >> 4) + 1) *	((surf->extents[1] >> 4) + 1);
			}
			r += (float) ((int) ((((((((r11 - r10) * dsfrac) >> 4) + r10) 
...
int RecursiveLightPoint_RGB (vec3_t color, mnode_t *node, vec3_t start, vec3_t end) {
	float		front, back, frac;
	vec3_t		mid;
loc0:
	if (node->contents < 0)
...
// calculate mid point
	...
	// optimized recursion
	if ((back < 0) == (front < 0)) {
//		return RecursiveLightPoint_RGB (color, node->children[front < 0], start, end);
		node = node->children[front < 0];
		goto loc0;
	}
...
// go down front side
	if (RecursiveLightPoint_RGB (color, node->children[front < 0], start, mid))
	...
	// go down back side
		return RecursiveLightPoint_RGB (color, node->children[front >= 0], mid, end);
	}
}
...
int R_LightPoint (vec3_t p) {
	...
	if (lightmap_bytes == 3) {
		...
		RecursiveLightPoint_RGB (lightcolor, cl.worldmodel->nodes, p, end);	
		return ((lightcolor[0] + lightcolor[1] + lightcolor[2]) * (1.0f / 3.0f));
	} else {
		if ((r = RecursiveLightPoint (cl.worldmodel->nodes, p, end)) == -1)
			r = 0;
		return r;
	}
}
