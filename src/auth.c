...
static void Auth_CheckFServerResponse (char *s, int flags, int offset) {
	...
	response = Auth_CheckString("QW262 f_server response: ", s, flags, offset, &slot, data, sizeof(data));
...
static void Auth_UnauthenticateClient(int slot) {
	if (strncmp(Info_ValueForKey(cl.players[slot].userinfo, "*client"), "QW262", 5) && auth_validateclients.value != 2)
		return;

	cl.players[slot].validated = false;
	cl.players[slot].f_server[0] = 0;
	if (auth_warninvalid.value)
		Con_Printf("Warning Invalid Client: User %s  (userid: %d)\n", cl.players[slot].name, Player_SlottoId(slot));
}
...
static void Auth_CheckAuthResponse (char *s, int flags, int offset) {
	...
	response = Auth_CheckString("QW262 version ", s, flags, offset, &slot, NULL, 0);
	if (response == AUTH_NOTHING || (!cls.demoplayback && slot == cl.playernum))
		return;
...
static void Auth_Verify_Clients_f(void) {
	...
	if (!Modules_SecurityLoaded()) {
		Con_Printf("Security module not initialized\n");
		return;
	}
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		if (cl.players[i].name[0] && !cl.players[i].spectator && (cls.demoplayback || i != cl.playernum)) {
			if (auth_validateclients.value != 2 && strncmp(Info_ValueForKey(cl.players[i].userinfo, "*client"), "QW262", 5)) {
	...
	if (authClientsCount || auth_validateclients.value == 2) {
		Con_Printf ("\x02" "Authenticated QW262 Clients:\n");
	...
	if (auth_validateclients.value != 2) {
		...
		if (unauthClientsCount) {
			Con_Printf ("\x02" "Unauthenticated QW262 Clients:\n");
		...
		if (otherClientsCount) {
			Con_Printf ("\x02" "Non QW262 Clients:\n");
...
void Auth_Init(void) {
	Cvar_RegisterVariable(&auth_viewcrc);
	
	Cvar_RegisterVariable(&auth_validateclients);
	Cvar_RegisterVariable(&auth_warninvalid);
