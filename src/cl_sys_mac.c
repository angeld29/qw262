...
*/
// cl_sys_mac.c -- Macintosh system driver
...
char *kAppName = "glqwcl262";

// The following are needed for activate/deactivate
extern qboolean video_restart;
...
static void Sys_Deactivate (void)
{
	Con_DPrintf ("Sys_Deactivate...\n");
	
	// This will (sometimes) happen with Sys_SwitchToFinder_f
	if (background) {
		Con_DPrintf ("Already deactivated!\n");
		return;
	}
...
static void Sys_Activate (void)
{
	...
	Con_DPrintf ("Sys_Activate...\n");
	
	if (!background) {
		Con_DPrintf ("Already active!\n");
		return;
	}
...
void Sys_SwitchToFinder_f (void)
{
	...
	if( Sys_FindProcess( kFinderSignature, kFinderType, &finderProcess ) == noErr)
		...
		Con_Printf ("Couldn't activate finder process.\n");
...
int main (int argc, char *argv[])
{
	...
	if (j && (j+1 < com_argc))
		parms.memsize = Q_atof (com_argv[j+1]) * 1024;

	j = COM_CheckParm ("-mem");
	if (j && (j+1 < com_argc))
		parms.memsize = Q_atof (com_argv[j+1]) * 1024 * 1024;

	parms.membase = malloc(parms.memsize);

	Sys_Printf ("Attempting to allocate %i bytes\n", parms.memsize);
	if (!parms.membase || parms.memsize < 12*1024*1024)// Should run with a 8MB hunk
		Sys_Error("Not enough memory to launch app."); // This doesn't return
