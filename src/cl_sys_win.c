...
qboolean OnChange_sys_disableWinKeys(cvar_t *var, const char *string);
cvar_t	sys_disableWinKeys = {"sys_disableWinKeys", "0", 0, OnChange_sys_disableWinKeys};
qboolean OnChange_sys_highpriority(cvar_t *, const char *string);
...
qboolean OnChange_sys_highpriority(cvar_t *var, const char *s)
{
	int ok, q_priority;
	char *desc;
	float priority;
...
{
	va_list argptr; 
// BorisU: made non-static. Extended to 3K to handle rcon status output
	char data[3072]; 
	int fd;
