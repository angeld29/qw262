...
static vec3_t	playerbeam_end; // for truelightning
...
// BorisU -->
	// light
		if (r_explosionlight.value) {
			dl = CL_AllocDlight (0);
			VectorCopy (pos, dl->origin);
			dl->radius = 75 + 100 * bound(0, r_explosionlight.value, 1);
			dl->die = cl.time + 0.5;
			dl->decay = 300;
			dl->type = dlightColor(r_explosionlightcolor.value, lt_redblue, true);;
		}
// <-- BorisU
