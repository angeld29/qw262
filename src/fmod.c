...
qboolean FMod_IsModelModified(char *name, int flags, byte *buf, int len) {
	...
	if (!Q_stricmp(gamedirfile, "ctf") && !strcmp(name, "progs/player.mdl"))
		name = "progs/player.mdl_ctf_";

	p = Security_IsModelModified(name, flags, buf, len);
//	Con_Printf("FMod_IsModelModified {%s} %i, %x \n", name, flags, Com_BlockChecksum(buf, len));

	if (!VerifyData(p))
		return failsafe;

	n = (int *) (p->buf);
//	Con_Printf ("returned %i\n", *n);
	retval = *n ? true : false;
	return retval;
}
...
void FMod_CheckModel(char *name, void *buf, int len) {
	...
	for (i = 0; i < MAX_CHECK_MODELS && i < check_models_num; i++) {
		...
		if (relevent && !Q_stricmp(name, check_models[i].name))
			break;
	}
	if (i >= check_models_num)
		return;
//	Con_Printf("checking %s %i\n",name, len);
