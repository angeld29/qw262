...
//		pixels = mt->width*mt->height/64*85;
		...
		if (!strncmp(mt->name,"sky",3))
			...
			if (mt->name[0] == '*') {		// we don't brighten turb textures
				if (cls.allow24bit & ALLOW_24BIT_TEXTURES)
					tx->gl_texturenum = loadtexture_24bit(mt->name, LOADTEX_BSP_TURB);
				if (tx->gl_texturenum == 0)
					tx->gl_texturenum = GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx+1), true, false, false, 1);

			} else {
				if (cls.allow24bit & ALLOW_24BIT_TEXTURES) {
					tx->gl_texturenum = loadtexture_24bit(mt->name, LOADTEX_BSP_NORMAL);
					tx->fb_texturenum = fb_texnum;
					tx->gl_lumitex = luma_texnum;
				}
				if (tx->gl_texturenum == 0) {
					tx->gl_texturenum = GL_LoadTexture (mt->name, tx->width, tx->height, (byte *)(tx+1), true, false, true, 1);
					tx->fb_texturenum = tx->gl_lumitex = 0;
					if (Img_HasFullbrights((byte *)(tx+1), tx->width*tx->height))
						tx->fb_texturenum = GL_LoadTexture (va("@fb_%s", mt->name), tx->width, tx->height, (byte *)(tx+1), true, 2, false, 1);
				}
			}
		}
	}
...
// fuh -->
static char *litfilename;
static byte *LoadColoredLighting(char *name)
{
	qboolean system;
	byte *data;
...
void Mod_LoadLighting (lump_t *l) {
	...
	data = LoadColoredLighting(loadmodel->name);
	
	if (data && l->filelen) {
		if (data[0] == 'Q' && data[1] == 'L' && data[2] == 'I' && data[3] == 'T') {
			if ((lit_ver = LittleLong(((int *)data)[1])) == 1) {
				if (com_filesize-8 == l->filelen*3) {
					if (developer.value || cl_warncmd.value)
						Con_Printf("Static coloured lighting loaded (%s)\n", COM_SkipPath(litfilename));
					...
					Con_Printf("Wrong .lit file %s, discarded\n", litfilename);
			} else
...
void Mod_LoadFaces (lump_t *l)
{
	...
	// lighting info
		...
		if (i == -1)
			...
			out->samples = loadmodel->lightdata + i * lightmap_bytes;
...
byte		aliasbboxmins[3], aliasbboxmaxs[3]; // Vic
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
	...
	for (i=0 ; i<numskins ; i++)
	{
		if (pskintype->type == ALIAS_SKIN_SINGLE) {
			...
			COM_StripExtension(COM_SkipPath(loadmodel->name), model);
			...
			if (cls.allow24bit & ALLOW_24BIT_TEXTURES) {
				...
				loadtexture_24bit(name, LOADTEX_ALIAS_MODEL);
			...
			if (pheader->gl_texturenum[i][0] == 0) {
				pheader->gl_texturenum[i][0] = pheader->gl_texturenum[i][1] =
				pheader->gl_texturenum[i][2] = pheader->gl_texturenum[i][3] =
				GL_LoadTexture (name, pheader->skinwidth, pheader->skinheight, (byte *)(pskintype + 1), true, false, false, 1);
				...
				if (Img_HasFullbrights((byte *)(pskintype + 1),	pheader->skinwidth*pheader->skinheight))
					...
					GL_LoadTexture (va("@fb_%s", name), pheader->skinwidth, pheader->skinheight, (byte *)(pskintype + 1), true, 2, false, 1);	
				else					
					pheader->fb_texturenum[i][0] = 	pheader->fb_texturenum[i][1] =
			...
			for (j=0 ; j<groupskins ; j++)
			{
					...
					pheader->gl_texturenum[i][j&3] = 
						...
						pheader->skinheight, (byte *)(pskintype), true, false, false, 1);	
					...
					if (Img_HasFullbrights((byte *)(pskintype),	pheader->skinwidth*pheader->skinheight))
						pheader->fb_texturenum[i][j&3] =
							...
							pheader->skinheight, (byte *)(pskintype), true, 2, false, 1);	
					else
...
// Angel --> Hack for headless model in TF2003
	if(!strcmp(mod->name, "progs/headless.mdl"))
		mod->modhint = MOD_PLAYER;
// <-- Angel
	start = Hunk_LowMark ();
...
// BorisU -->
	if (!strcmp(mod->name, "progs/grenade2.mdl"))
...
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	*ppframe = pspriteframe;
	...
	if (cls.allow24bit & ALLOW_24BIT_TEXTURES) {
		pspriteframe->gl_texturenum = loadtexture_24bit(name, LOADTEX_SPRITE);
	}

	if (pspriteframe->gl_texturenum == 0) {
		sprintf (name, "%s_%i", loadmodel->name, framenum);
		pspriteframe->gl_texturenum = GL_LoadTexture (name, width, height, (byte *)(pinframe + 1), true, true, false, 1);
	}
