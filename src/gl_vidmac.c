...
void MacSetupScreen ()
{
	...
	if (wantedAttributes.frequency != foundAttributes.frequency)
		Con_Printf ("WARNING: Refresh is out of range. Using closest match...\n");
		
	if (wantedAttributes.displayWidth != foundAttributes.displayWidth)
		Con_Printf ("WARNING: Screen width is out of range. Using closest match...\n");

	if (wantedAttributes.displayHeight != foundAttributes.displayHeight)
		Con_Printf ("WARNING: Screen height is out of range. Using closest match...\n");
...
void CheckTextureExtensions (void)
{
	...
	// Some cards report this as available in classic, but I was under
	// the impression that it's only available under OSX 10.1
	if ( strstr(gl_extensions, "GL_EXT_texture_filter_anisotropic ") )
		// True if gl_ext_maxanisotropic > 0
		glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &gl_ext_anisotropic_max);
...
void GL_PrintAvailableExtensions (void)
{
	if (gl_ext_multitexture_units > 1)
		Con_Printf ("%i multitexture units available.\n", gl_ext_multitexture_units);

	if (gl_ext_anisotropic_max)
		Con_Printf ("Anisotropic filtering available.\n");

	if (gl_ext_packedpixel)
		Con_Printf ("Packed pixels available.\n");
		
	if (gl_ext_texture_env_combine)
		Con_Printf ("Texture combine env available.\n");
		
	if (gl_ext_texture_env_add)
		Con_Printf ("Texture add env available.\n");
	
	if (gl_ext_compiled_vertex_array)
		Con_Printf ("Compiled vertex arrays available.\n");
}
...
void VID_SetDeviceGammaRamp (unsigned short ramps[3][256])
{
	...
	for (i = 0; i < 256; ++i)
	{
		gamma[0][i] = ramps[0][i] / 65535.0;
		gamma[1][i] = ramps[1][i] / 65535.0;
		gamma[2][i] = ramps[2][i] / 65535.0;
	}
...
{
	vid_gammaworks = true;
	Con_Printf("Hardware Gamma enabled\n");
	vid_hwgamma_enabled = vid_hwgammacontrol.value && vid_gammaworks
		&& !background && !inwindow;
}
...
void VID_DescribeModes_f (void)
{
	...
	{
		Con_Printf ("Requested screen not found\n");
		return;
	}
	else
	{
...
void VID_DescribeCurrentMode_f (void)
{
	...
	Con_Printf (" Mode: %i (%ix%i,%iHz)\n", gScreen.profile->mode,
										  	gScreen.mode->width,
										  	gScreen.mode->height,
										  	(int)gScreen.mode->refresh>>16);
	
	Con_Printf (" Fullscreen Setup: ");
	...
	Con_Printf (" Fullscreen Depth: %i bit\n", gScreen.profile->colorbits);
	Con_Printf (" Texture Depth: %i bit\n", gScreen.profile->texturebits);
	Con_Printf (" Windowed: ");
...
// This looks much worse than it really is :)
	...
	{
		if (SCR_ModalMessage ((char *)"Applying the requested video\nsettings will abort the current game.\n\nDo you want continue? Y/N"))
			CL_Disconnect_f ();
		else
			return;
	}
...
void VID_Init (unsigned char *palette)
{
	...
	Cvar_RegisterVariable (&gl_vid_windowed);
	Cvar_RegisterVariable (&gl_vid_screen);
	Cvar_RegisterVariable (&gl_vid_colorbits);
