...
extern qboolean	mouseactive, mouse_show;  // from in_win.c
static HICON	hIcon;
...
//static qboolean fullsbardraw = false; // remove? BorisU
...
unsigned short (*currentgammaramp)[256] = NULL;
...
qboolean VID_SetFullDIBMode (int modenum)
{
	...
	if (!leavecurrentmode)
	{
		...
		// kazik -->
		i = COM_CheckParm ("-freq");
		if (i)
		{
			if (i < com_argc-1)
			{
				gdevmode.dmDisplayFrequency = Q_atoi (com_argv[i+1]);
		...
		if ( ret != DISP_CHANGE_SUCCESSFUL)
			Sys_Error ("Couldn't set fullscreen DIB mode (%i)", ret);
	}
...
// BorisU -->
qboolean update_vsync = true; // Will be set OFF at startup

qboolean OnVidVsyncChange(cvar_t *var, const char *string) {
	update_vsync = true;
	return false;
}

cvar_t	vid_vsync = {"vid_vsync", "0", 0, OnVidVsyncChange};

typedef BOOL (APIENTRY *SWAPINTERVALFUNCPTR)(int);
SWAPINTERVALFUNCPTR wglSwapIntervalEXT = NULL;


void CheckVsyncControlExtension(void) {
	if (GL_CheckExtension("WGL_EXT_swap_control")) {
...
static qboolean hwgamma1dontwork = false;
qboolean V_OnHWgammaChange (cvar_t *var, const char *value)
{
	...
	if (newhwgamma != 1 && mtfl==1)
		return true;

	if (newhwgamma != 2 && mtfl==2)
		return true;

	if (newhwgamma == 1 && hwgamma1dontwork)
		return true;
...
// handle the mouse state when windowed if that's changed
	...
	{
		if (!_windowed_mouse.value || mouse_show) {
			if (windowed_mouse)	{
				IN_DeactivateMouse ();
				IN_ShowMouse ();
...
void VID_SetDeviceGammaRamp (unsigned short ramps[3][256])
{
	...
	if (vid_gammaworks) {
		if (vid_hwgamma_enabled) {
			ret = SetDeviceGammaRamp (maindc, ramps);
			if (ret == 0) {
				if (vid_hwgammacontrol.value == 1) {
					if (mtfl == 1)
						mtfl = 2;
					hwgamma1dontwork = true;
					Con_Printf("SetDeviceGammaRamp returned error\n"
								"vid_hwgammacontrol set to 2\n");
					Cvar_SetValue(&vid_hwgammacontrol,2);
				} else if (mtfl == 2) { // still has error, contrast too high
...
BOOL bSetupPixelFormat(HDC hDC)
{
	...
	// BorisU: ask 24bit as some nVidia drivers set z-buffer depth to 16 if asked to set 32
	24,				// 32-bit z-buffer 
	0,				// no stencil buffer
	...
	if (COM_CheckParm("-zbuf")) {
		pfd.cDepthBits = Q_atoi(com_argv[COM_CheckParm("-zbuf")+1]);
	}
...
// enable/disable sound on focus gain/loss
	...
	if (fActive) {
		...
		else if ((modestate == MS_WINDOWED) && _windowed_mouse.value && !mouse_show && key_dest == key_game) {
			IN_ActivateMouse ();
			IN_HideMouse ();
		}
	}
...
void M_OnMHooksMessage(WORD _nEvents);
...
// Sergio (printscr) -->
		case WM_HOTKEY:
			if (wParam == id_PrintScr) 
				Key_Event(K_PRINTSCR, true);
			break;
// <-- Sergio
		...
		case WM_SYSKEYDOWN:
// BorisU -->
			if (!(lParam & 0xFF0000)) { // Mouseware emulated button hack
				lParam |= MapVirtualKey(wParam,0)<<16;
			}
// <-- BorisU
...
// <-- Massa
		...
		default:
			if (uMsg == MH_SinkMsg)
			{
				Con_DPrintf("MHooks: wParam=%x lParam=%x\n", wParam, lParam);
				MH_OnHookEvent(wParam);
				break;
			}
...
void	VID_Init (unsigned char *palette)
{
	...
	Cvar_RegisterVariable (&vid_hwgammacontrol); // Tonik
