...
void IN_Init (void)
{
	...
	Cmd_AddCommandTrig ("m_show", Release_Cursor_f);
	Cmd_AddCommandTrig ("m_hide", Capture_Cursor_f);

	Cvar_RegisterVariable (&m_filtercutoff);
...
static void IN_OverrideSystemPrefs (qboolean state)
{
    ...
    {
    	Con_DPrintf ("NXOpenEventStatus Failed!\n");
        return;
	}
...
//if (temp1.value) Con_Printf ("\3IX: %3.2f IY: %3.2f\n", posx, posy);
...
//if (temp1.value) Con_Printf ("\3FX: %3.2f FY: %3.2f\n", posx, posy);
