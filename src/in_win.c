...
cvar_t	m_forcewheel = {"m_forcewheel", "0"}; // fuh
...
qboolean	mouseactive, mouse_show;
...
cvar_t	cl_keypad = {"cl_keypad","0"}; // Tonik
...
void IN_SMouseRead(int *mx, int *my)
{
	...
	*my = y;
	...
	// serve wheel
	wheel_dn_count = bound(0, wheel_dn_count, 10);
	wheel_up_count = bound(0, wheel_up_count, 10);
...
void IN_SMouseInit(void)
{
	...
	if ((res = IDirectInputDevice_SetEventNotification(g_pMouse, m_event)) != DI_OK  
			&&  res != DI_POLLEDDEVICE)
		return;
...
void IN_StartupMouse (void)
{
	...
	mouse_buttons = 8; // fuh: up to 8 buttons
...
// set the mouse state appropriately
	...
	cursor_pos.x = window_center_x;
	cursor_pos.y = window_center_y;
}

// BorisU -->

int MW_buttons_mapping[5] = {K_MOUSE4, K_MOUSE5, K_MOUSE6, K_MOUSE7, K_MOUSE8};
...
===========
*/
void M_Map_Button_f (void)
{
	int mousebutton = Q_atoi(Cmd_Argv(1));
	int mapped = Key_StringToKeynum(Cmd_Argv(2));

	if (Cmd_Argc() != 3) {
		Con_Printf("usage: m_map_button mousebutton key\n");
		return;
	}
	
	if (mousebutton < 4 || mousebutton > 8) {
		Con_Printf("m_map_button: invalid mouse button\n");
		return;
	}
	
	if (mapped == -1) {
		Con_Printf("m_map_button: invalid key\n");
		return;
	}
	if (mapped == K_CTRL || mapped == K_ALT || mapped == K_SHIFT)
		mapped++;

	MW_buttons_mapping[mousebutton-4] = mapped;
	Con_Printf("Mouse button %d mapped to %s\n", mousebutton, Key_KeynumToString(mapped));
}

void M_ReInit_f (void)
{
	IN_DeactivateMouse ();
	IN_ActivateMouse ();
}
...
void IN_Init (void)
{
	...
	Cvar_RegisterVariable (&m_forcewheel);	// fuh

	// keyboard variables 
	Cvar_RegisterVariable (&cl_keypad);		// Tonik
...
// BorisU -->
	Cmd_AddCommand ("m_map_button", M_Map_Button_f);
	Cmd_AddCommand ("m_reinit", M_ReInit_f);
	Cmd_AddCommandTrig ("m_show", M_Show_f);
	Cmd_AddCommandTrig ("m_hide", M_Hide_f);
// <-- BorisU
...
===========
*/
void IN_MouseButtons (int mstate)
{
	int		i;
...
void IN_MouseEvent (int mstate)
{
	...
	{
		if (!dinput)
			IN_MouseButtons (mstate);
	}
	else
	{
		POINT cur_pos;
		GetCursorPos (&cur_pos);
		Hud_MouseEvent (cur_pos.x - window_x, cur_pos.y - window_y, mstate);
	}
}
...
void IN_MouseMove (usercmd_t *cmd)
{
	...
	if (m_filter.value) {
		...
		f = bound(0, m_filter.value, 1) / 2.0;
		mouse_x = (mx * (1 - f) + old_mouse_x * f);
		mouse_y = (my * (1 - f) + old_mouse_y * f);
	} else {
		mouse_x = mx;
		mouse_y = my;
	}
