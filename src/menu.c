...
enum {m_none, m_main, m_singleplayer, m_load, m_save, m_multiplayer, m_setup, 
	m_net, m_demos, m_options, m_video, m_keys, m_help, m_quit, m_serialconfig, 
...
cvar_t cl_demoplayer_sort = {"cl_demoplayer_sort", "0"};
...
void M_DrawBar (int x, int y, int len, int c)
{
	Draw_Fill(x + ((menuwidth - 320)>>1), y + m_yofs, len*8+1, 9, c);
}
...
void M_Main_Draw (void)
{
	...
	f = (int)(curtime * 10)%6; // Sergio - realtime2
...
// cursor
	M_DrawCharacter (64, 40 + m_multiplayer_cursor*8, 12+((int)(curtime*4)&1)); // Sergio - realtime2
}
...
#define START_MAX_DEMO_FILES 32
...
static void ReadDir (void)
{
	...
	do {
		...
		stat (va("%s/%s/%s", com_basedir, demodir, filename), &fileinfo);
		...
		if (is_dir) {
			...
			i = strlen(filename);
			if (i < 5 || (Q_stricmp(filename+i-4, ".qwd")
				&& Q_stricmp(filename+i-4, ".qwz")
				&& Q_stricmp(filename+i-4, ".mvd")))
				continue;
			type = 0;
		}
		...
		for (i=0 ; i<numfiles ; i++)
		{
			...
			if (type > dir[i].type)
				break;
			if (cl_demoplayer_sort.value == 0){
				if (Q_stricmp (filename, dir[i].name) < 0)
					break;
			} else if (cl_demoplayer_sort.value == 1) {
				if (size < dir[i].size)
					break;
			}
		}
		...
		for (i=numfiles-1 ; i>pos ; i--)
			dir[i] = dir[i-1];
		dir[i].name = strdup(filename);
		dir[i].type = type;
		dir[i].size = size;
		if (numfiles == max_demo_files) {
			max_demo_files *= 2;
			dir = realloc(dir, max_demo_files*sizeof(direntry_t));
		}
#ifdef _WIN32
...
void M_Demos_Draw (void)
{
	...
	M_Print (8, 24, "\x1d\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1e\x1f \x1d\x1e\x1e\x1e\x1e\x1e\x1e\x1f");
	...
	for (i=0, y=32 ; i<numfiles-demo_base && i<MAXLINES ; i++, y+=8, d++)
	{
		...
		if (i == demo_cursor)
			M_DrawBar (8, y, 38, 214);
		if (d->type)
			M_PrintWhite (8, y, str);
		else
			M_Print (8, y, str);

		if (d->type == 1)
			M_PrintWhite (248, y, "  folder");
		else if (d->type == 2)
			M_PrintWhite (248, y, "    up  ");
		else if (d->type == 0)
			M_Print (248, y, toyellow(va("%7ik", d->size>>10)));
	}
	
//	M_DrawCharacter (8, 32 + demo_cursor*8, 12+((int)(realtime2*4)&1)); // Sergio - realtime2
}
...
void M_Demos_Key (int k)
{
	...
	case K_ENTER:
		...
		if (dir[demo_base + demo_cursor].type) {
			...
			{
				*prevdir = '\0'; // Sergio
				strlcat (demodir, "/", sizeof(demodir));
				strlcat (demodir, dir[demo_base + demo_cursor].name, sizeof(demodir));
			}
			demo_cursor = 0;
			ReadDir ();
		}
		...
		{
			key_dest = key_game;
			key_repeats[K_ENTER] = 0; // BorisU
			m_state = m_none;
			strcpy(prevdir, dir[demo_cursor+demo_base].name); // Sergio
	...
	default:
		{ // first letter search
			...
			for ( i = 1; i < numfiles; i++) {
				if (tolower(dir[(i+demo_cursor+demo_base)%numfiles].name[0]) == k) {
					demo_base = (i+demo_cursor+demo_base)%numfiles;
					demo_cursor = 0;
					break;
				}
			}
...
// cursor
	M_DrawCharacter (200, 32 + options_cursor*8, 12+((int)(curtime*4)&1)); // Sergio - realtime2
}
...
// search for known bindings
	...
	if (bind_grab)
		...
		M_DrawCharacter (130, 48 + keys_cursor*8, 12+((int)(curtime*4)&1)); // Sergio - realtime2
}
...
//    0123456789012345678901234567890123456789
	"0       Ñ×”˜” by ”wð‘ÂorisÕ",
	"0            Revision " VSTR2(MINOR_VERSION),
	"1        based on QuakeWorld",
...
void M_Init (void)
{
...
#endif
	Cvar_RegisterVariable (&cl_demoplayer_sort);
	...
	max_demo_files = START_MAX_DEMO_FILES;
	dir = calloc(max_demo_files, sizeof(direntry_t));
}
...
void M_Draw (void)
{
	...
	if (scr_scaleMenu.value) {
		menuwidth = 320;
		menuheight = min (vid.conheight, 240);
...
{
// BorisU -->
// keypad works even if cl_keypad == 1
	switch (key) {
