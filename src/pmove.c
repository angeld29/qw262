...
#ifndef SERVERONLY
qboolean OnJumpFixChange (cvar_t *var, const char *value)
{
	float f = Q_atof(value);
	if (f)
		Con_Printf("Client jump-fix ON\n");
	else	
		Con_Printf("Client jump-fix OFF\n");
	return false;
}

cvar_t	pm_jumpfix = {"pm_jumpfix","0", 0, OnJumpFixChange};
#else
cvar_t	pm_jumpfix = {"pm_jumpfix","0", CVAR_SERVERINFO};

qboolean OnBunnySpeedCapChange (cvar_t *var, const char *value)
{
	movevars.bunnyspeedcap = Q_atof(value);
	return false;
}

cvar_t	pm_bunnyspeedcap = {"pm_bunnyspeedcap","0", CVAR_SERVERINFO, OnBunnySpeedCapChange};
#endif
