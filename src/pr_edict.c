...
char *PR_ValueString (etype_t type, eval_t *val)
{
	...
	{
	case ev_string:
		sprintf (line, "%s", PR1_GetString(val->string));
		break;
	case ev_entity:	
	...
	case ev_function:
		...
		sprintf (line, "%s()", PR1_GetString(f->s_name));
		break;
	case ev_field:
		def = ED_FieldAtOfs ( val->_int );
		sprintf (line, ".%s", PR1_GetString(def->s_name));
		break;
	case ev_void:
		sprintf (line, "void");
		break;
...
char *PR_GlobalString (int ofs)
{
	...
	else
	{
		...
		sprintf (line,"%i(%s)%s", ofs, PR1_GetString(def->s_name), s);
	}
...
char *PR_GlobalStringNoContents (int ofs)
{
	...
	if (!def)
		...
		sprintf (line,"%i(%s)", ofs, PR1_GetString(def->s_name));
...
// flush the non-C variable lookup cache
	...
	Q_snprintfz(name,sizeof(name), "%s.dat", sv_progsname.string);
