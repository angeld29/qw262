...
void R_AliasTransformAndProjectFinalVerts (finalvert_t *fv, stvert_t *pstverts)
{
	...
	// projection
		...
		fv->v[0] = ((DotProduct(interpolated_verts, aliastransform[0]) +
				aliastransform[0][3]) * zi) + aliasxcenter;
		fv->v[1] = ((DotProduct(interpolated_verts, aliastransform[1]) +
	...
	// lighting
		VectorInterpolate(r_avertexnormals[pverts1->lightnormalindex], r_framelerp,
			r_avertexnormals[pverts2->lightnormalindex], interpolated_norm);
		lightcos = DotProduct (interpolated_norm, r_plightvec);
		temp = r_ambientlight;
...
{
	int lnum, ambientlight, shadelight;
	float add, fbskins;
	vec3_t dist;
...
void R_AliasDrawModel (entity_t *ent)
{
	...
	pmodel = ent->model;
	paliashdr = (aliashdr_t *)Mod_Extradata (pmodel);
	pmdl = (mdl_t *)((byte *)paliashdr + paliashdr->model);

	if (ent->frame >= pmdl->numframes || ent->frame < 0) {
		Con_DPrintf ("R_AliasDrawModel: no such frame %d\n", ent->frame);
		ent->frame = 0;
	}
	if (ent->oldframe >= pmdl->numframes || ent->oldframe < 0) {
		Con_DPrintf ("R_AliasDrawModel: no such oldframe %d\n", ent->oldframe);
		ent->oldframe = 0;
	}
