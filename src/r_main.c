...
qboolean OnGHChange (cvar_t *var, const char *value)
{
	int v = atoi(value);
	if ( v > 50 || v < 1)
		return true;
...
cvar_t	r_graphheight = {"r_graphheight","15", 0, OnGHChange};
cvar_t	r_clearcolor = {"r_clearcolor","2"};
cvar_t	r_fastsky = {"r_fastsky","0"}; // Tonik
cvar_t	r_drawflame = {"r_drawflame","1"}; // Tonik
cvar_t	r_waterwarp = {"r_waterwarp","1"};
cvar_t	r_drawentities = {"r_drawentities","1"};
cvar_t	r_lerpframes = {"r_lerpframes", "1"}; // fuh
...
cvar_t	r_fullbrightskins = {"r_fullbrightskins","0"}; // Tonik
...
// get stack position so we can guess if we are going to overflow
	...
	Cvar_RegisterVariable (&r_fastsky); // Tonik
	Cvar_RegisterVariable (&r_drawflame); // Tonik
	Cvar_RegisterVariable (&r_waterwarp);
	Cvar_RegisterVariable (&r_drawentities);
	Cvar_RegisterVariable (&r_lerpframes); // fuh
...
// adjust console
	if (scr_con_current && (scr_con_current < pvrectin->height)
		&& (pvrect->y < scr_con_current))
	{
		int delta = scr_con_current - pvrect->y;
...
extern cvar_t	cl_crosshair_hack;
	
	if (!r_drawviewmodel.value || !cl.viewent.current.modelindex ||
		(r_fov_greater_than_90 && r_drawviewmodel.value == 2.0 ) || 
		!Cam_DrawViewModel())
		return;
