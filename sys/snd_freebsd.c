...
SNDDMA_Init(void)
{
	...
	{
		Con_Printf("Sorry, but your soundcard cannot do memory-mapped I/O\n");
		close(audio_fd);
		return (0);
	}
	...
	{
		perror(snddev);
		Con_Printf("Um, cannot do GETOSPACE?\n");
		close(audio_fd);
		return (0);
	}
	...
	if (shm->samplebits != 16 && shm->samplebits != 8)
	{
		...
		if (tmp & AFMT_S16_LE)
			...
			Con_Printf("Could not set sample format\n");
			close(audio_fd);
			return (0);
		}
	}
	...
	{
		perror(snddev);
		Con_Printf("Could not set %s to stereo = %d\n", snddev, shm->channels);
		close(audio_fd);
		return (0);
	}

	Con_Printf("Sound using stereo = %d\n", shm->channels);
	...
	{
		perror(snddev);
		Con_Printf("Could not set %s speed to %d\n", snddev, shm->speed);
		close(audio_fd);
		return (0);
	}

	Con_Printf("Sound on speed = %d\n", shm->speed);
	...
	Con_Printf("%s is set to %d-bit format.\n", snddev, shm->samplebits);
