...
qboolean SNDDMA_Init(void)
{
	...
	if (!gFrequency)
	{
		Con_Printf ("WARNING: Couldn't read sound rate preference\n");
		gFrequency = 22050;
	}
	...
	if (err != noErr)
	{
		Con_Printf("Failed to allocate sound channel. OS error %d\n",err);
		return false;
	}
	...
	{
		Con_Printf("Failed to allocate sound channel completion routine.\n");
		return false;
	}
...
#undef free
		...
		{
			Con_Printf("Failed to allocate sound channel memory.\n");
			return false;
		}
...
//		gHeader.dbhBufferPtr[i]->dbUserInfo[1] = SetCurrentA5 ();
	...
	if (err != noErr)
	{
		Con_Printf("Failed to start sound. OS error %d\n",err);
		return false;
	}
